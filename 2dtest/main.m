...
%We obtain three steady states x1,x3,x3 of the simulated system (parameter is setting as a0=1,b0=1,S0_{ij}=0.5) as simulated experimental data
%This is computed with steadystates.m
%Our aim is to optimize the parameter a,b,S so that the steady states are close to the simulated experimental data
...
%% Decrease the learning rate(l) and rerun if there's a phase change
if size(SS,1)<3
    if pflag > 10
        break
    end
...
%s1,s2,s3 is the computed steady state
